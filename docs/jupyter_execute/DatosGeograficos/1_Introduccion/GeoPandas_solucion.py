...
# [![Open In colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/institutohumai/cursos-python/blob/master/DatosGeograficos/1_Introduccion/GeoPandas_solucion.ipynb)
...
punto = Point((-58.502625, -34.656023))


# In[44]:


punto_mataderos = pd.DataFrame(data = {'id': [1]}) # Armo un DataFrame

punto_mataderos_gdf = geopandas.GeoDataFrame(df, crs="EPSG:4326", geometry=[punto])

punto_mataderos_gdf.to_crs("EPSG:3395")

fig, ax = plt.subplots()
palermo = barrios[barrios["BARRIO"] == "MATADEROS"]
palermo.to_crs(epsg = 3395).plot(ax=ax)
punto_mataderos_gdf.to_crs("EPSG:3395").plot(ax=ax, color="red")
...
flores_gkba = barrios_gkba[barrios_gkba["BARRIO"] == "FLORES"]


# In[89]:


area_flores = flores_gkba.area
...
interseccion_flores = geopandas.overlay(bomberos_gkba_buff, flores_gkba, how = "intersection")


# In[100]:


interseccion_flores["const"] = 0
m2_cubiertos = interseccion_flores.dissolve(by="const").area.iloc[0]
...
porc_cubierto = m2_cubiertos/area_flores.iloc[0]*100 
...
fig, ax = plt.subplots()
flores_gkba.plot(ax=ax)
interseccion_flores.plot(ax=ax, color="red")
...
# 1 - la unión mantiene todos los objetos geográficos de ambos geodataframes
geopandas.overlay(barrios_gkba, bomberos_gkba_buff, how="union").plot()


# In[111]:


# 2 - son los barrios habiéndoles removido la parte que se superpone con bomberos_gkba_buff
geopandas.overlay(barrios_gkba, bomberos_gkba_buff, how="difference").plot()
...
recoleta_cen = barrios_gkba.query("BARRIO == 'RECOLETA'").centroid.iloc[0]


# In[133]:


monte_cen = barrios_gkba.query("BARRIO == 'MONTE CASTRO'").centroid.iloc[0]


# In[136]:


mas_cercano = bomberos_gkba.geometry.apply(lambda x: "RECOLETA" if x.distance(recoleta_cen) < x.distance(monte_cen) else "MONTE CASTRO")


# In[137]:


mas_cercano.value_counts()
