...
# [![Open In colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/institutohumai/cursos-python/blob/master/DatosGeograficos/2_Joins_y_Viz/2_Joins_Espaciales_solucion.ipynb)
...
# usando within comisarias va primero, porque queremos ver si está dentro de un barrio
serie_a = geopandas.sjoin(comisarias, barrios, op="within", how="left")["BARRIO"].value_counts().sort_index()


# In[15]:


# usando contains barrios va primero, porque queremos ver si contiene a la comisaria
serie_b = geopandas.sjoin(barrios, comisarias, op="contains", how="right")["BARRIO"].value_counts().sort_index()
...
serie_a = geopandas.sjoin(comisarias, divisiones, op="within", how="left")["division"].value_counts().sort_index()


# In[ ]:


serie_b = geopandas.sjoin(divisiones, comisarias, op="contains", how="right")["division"].value_counts().sort_index()


# In[ ]:


serie_a.equals(serie_b)
